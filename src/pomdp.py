...
class POMDPEnvironment:
    ...
    def update_belief(self, prev_belief, action_num, observation_num):
        ...
        for b_s in b_new_nonnormalized:
            b_new.append(b_s/total) #todo: er stonden [] rond b_s/total
...
class POMDPPolicy:
    ...
    Attributes:
        ...
        adapted to read erwin walraeven's .alpha files
    """
    ...
    def __init__(self, filename):
        ...
        for i in range(0,int(len(contents)/2)):
            self.action_nums.append(contents[2*i])
            vals = []
            for val in contents[2*i + 1].split(" "):
